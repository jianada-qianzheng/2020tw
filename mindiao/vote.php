<?php
    ...
    for ($age=2;$age<=7;$age++  ){
        ...
        for ($city=1;$city<=22;$city++) {
            ...
            for ($gender=2;$gender>=1;$gender-- ){
                
                $sql2="";
                
                if($age==7){
                     $sql2 = "SELECT vote,  count( *) as votecount  FROM vote2 where active=1 and hukou='$city' and age>69  and sex='$gender' and (TIMESTAMPDIFF(DAY, date, NOW()) <7) group by vote order by vote ";
                }else{
                    $ageRange1=$age*10-1;
                    $ageRange2=$age*10+10;
                ...
                $index = $total*$parts[$age-2][$city-1][$gender-1]/$groupTotal/19286045;
                ...
                print($parts[$age-2][$city-1][$gender-1]);
    ...
    $percentage1=1;//$voteCount1/($voteCount1+$voteCount2+$voteCount3+$voteCount4+$voteCount5+$voteCount6);
    $percentage2=1;//$voteCount2/($voteCount1+$voteCount2+$voteCount3+$voteCount4+$voteCount5+$voteCount6);
    $percentage3=1;//$voteCount3/($voteCount1+$voteCount2+$voteCount3+$voteCount4+$voteCount5+$voteCount6);
    $percentage4=1;//$voteCount4/($voteCount1+$voteCount2+$voteCount3+$voteCount4+$voteCount5+$voteCount6);
    $percentage5=1;//$voteCount5/($voteCount1+$voteCount2+$voteCount3+$voteCount4+$voteCount5+$voteCount6);
    $percentage6=1;$voteCount6/($voteCount1+$voteCount2+$voteCount3+$voteCount4+$voteCount5+$voteCount6);
    print("perceantage2");
    print($percentage2);
